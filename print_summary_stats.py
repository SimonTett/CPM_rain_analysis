...
for key in ['1km','5km', '1km_c5', '1km_c4']:
    ...
    topog = xarray.open_dataarray(CPMlib.radar_dir / 'topography' / f'topog_{key}.nc').sel(**CPMlib.carmont_rgn_OSGB)
    maxRain = maxRain.where(topog > 0.0)  # land points
    maxRain.load()  # need to load it to compute median
...
median_max = median_max.sort_index(axis=1)
...
with open(CPMlib.table_dir / 'max_rain.tex', 'w') as f:
    median_max.rename(columns=rename).rename_axis('').style. \
        ...
        caption="Regional Median Seasonal Max Total Rain (mm), over land,  for the JJA 2008-2023 period and, for the CPM,  without bias correction.", hrules=True
    )

## print out the areas and event_count in a multi-index
df_areas = pd.concat([ds.rename(k).drop_vars(['quantv','coarsen'],errors='ignore').to_dataframe() for k, ds in areas.items()], axis=1)
df_counts = pd.concat([ds.rename(k).drop_vars(['quantv','coarsen'],errors='ignore').to_dataframe() for k, ds in event_count.items()], axis=1)
...
with open(CPMlib.table_dir / 'event_stats.tex', 'w') as f:
    merged_df.rename(columns=rename).rename_axis('').style. \
        ...
        format(precision=0). \
        to_latex(f,   column_format='l'+'|ll'*6,
                 label='tab:event_stats', position='ht!',
                 caption="Area (km$^2$) and Event count/year for  CPM  and  RADAR datasets.", hrules=True
                 )
