...
# jobs for one ensemble are submitted dependant on the previpous job.
...
do
    ...
    mkdir -p $outdir
    decs=$(ls $dir| cut -f8 -d"_" | cut -c1-3|sort|uniq)
    #decs='199' # for testing
    for dec in $decs
    do
	decp=$(($dec+1))
	range=$dec"0-12-01 "
	...
	r1=$dec"0"
	cmd="./ens_seas_max.py  $dir/pr_*_1hr_$dec[0-9]*.nc $dir/pr_*_1hr_$decp[0]*.nc --region $rgn -o $outdir/CPM_pr_seas_max_ens$ens --monitor --verbose  --coarsen 2 --range $range --filter_bad --no_overwrite"
	#echo "cmd is $cmd"
	...
	#echo $run_cmd
	#eval $run_cmd
	#echo sbatch -p $Q -t $TIME -o $outfile $cmd
