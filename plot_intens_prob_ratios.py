...
nsamp=1000
temp_p2k_samp = temp_p2k.rvs(nsamp, random_state=123456)
temp_p2k_samp = xarray.DataArray(temp_p2k_samp, dims='sample')
...
for axis, rolling in zip(axs, [1, 4]):
    ...
    for index, name in enumerate(names):  # loop over different radars
        ...
        for color, period,temp in zip(['cornflowerblue', 'blue', 'red'],
                                      ['1980s', 'Today', '+2C'], [t_1980s, t_today, temp_p2k_samp]):
